...
// Componente para el pegado rápido de datos
function PasteDataForm({ addMultiplePassengers }) {
    ...
    const handleProcess = () => {
        ...
        const newPassengers = lines.map(line => {
            const parts = line.split(/[,;\t]/);
            if (parts.length < 2) return null;
            const name = parts[0].trim();
            const rut = parts[1].trim();
            if (name && rut) return { id: Date.now() + Math.random(), name, rut };
            return null;
        ...
        if (newPassengers.length > 0) {
            ...
            showNotification("No se pudo procesar el texto. Revise el formato (nombre, rut).", 'error');
        }
    };
    ...
    return (
        <div className="p-4 bg-white rounded-lg shadow-md border border-gray-200">
            ...
            <p className="text-sm text-gray-600 mb-3">Pegue la lista de pasajeros. Use una línea por persona, separando nombre y RUT con una coma (<code>,</code>).</p>
            <textarea
                value={pastedText}
            ...
            />
            <button onClick={handleProcess} className="mt-3 w-full flex justify-center items-center gap-2 px-4 py-2 bg-indigo-600 text-white font-semibold rounded-lg shadow-md hover:bg-indigo-700">
            ...
            {notification.show && <div className={`mt-3 p-3 rounded-md text-sm text-center ${notification.type === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{notification.message}</div>}
        </div>
    );
}

// Componente para el formulario de pasajero individual
...
// Componente para la lista de pasajeros
function PassengerList({ passengers, removePassenger }) {
  if (passengers.length === 0) {
    return <div className="p-4 text-center bg-white rounded-lg shadow-md border border-gray-200"><p className="text-gray-500">Aún no se han agregado pasajeros.</p></div>;
  }
  return (
    <div className="p-4 bg-white rounded-lg shadow-md border border-gray-200">
...
function PrintPreview({ reservation, passengers, closePreview }) {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center p-4 z-50">
            ...
            <div className="bg-white p-8 rounded-lg shadow-2xl w-full max-w-4xl relative print-container">
                <div className="text-center mb-6 border-b pb-4">
                    <img src="/logo.png" alt="Logo Cabañas Manuara" className="h-24 w-auto mx-auto mb-4 rounded-full" />
                ...
                <div className="space-y-2">
                    {passengers.length > 0 ? passengers.map(p => (<div key={p.id} className="flex justify-between items-center border-b py-2 px-2"><span className="text-gray-900">{p.name}</span><span className="text-gray-600 font-mono">{p.rut}</span></div>)) : <p className="text-gray-500">No hay pasajeros.</p>}
                </div>
                <div className="mt-8"><h2 className="text-xl font-semibold text-gray-800 mb-2">Notas:</h2><p className="text-gray-700 whitespace-pre-wrap border p-3 rounded-md min-h-[50px]">{reservation.notes || 'Sin notas adicionales.'}</p></div>
                ...
                <div className="mt-8 flex justify-end gap-4 no-print">
                    ...
                    <button onClick={() => window.print()} className="px-4 py-2 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 flex items-center gap-2"><Icon path="M6.72 13.829c-.24.03-.48.062-.72.096m.72-.096a42.415 42.415 0 0110.56 0m-10.56 0L6.34 18m10.94-4.171c.24.03.48.062.72.096m-.72-.096L17.66 18m0 0l.229 2.523a1.125 1.125 0 01-1.12 1.227H7.231a1.125 1.125 0 01-1.12-1.227L6.34 18m11.32 0zM9 11.625a1.125 1.125 0 112.25 0 1.125 1.125 0 01-2.25 0z" className="w-5 h-5"/>Imprimir / Guardar PDF</button>
                </div>
            </div>
        </div>
    );
...
function App() {
  ...
  useEffect(() => {
    ...
    } catch (error) { console.error("Error al cargar datos desde localStorage", error); }
  }, []);
  ...
  useEffect(() => {
    ...
    } catch (error) { console.error("Error al guardar datos en localStorage", error); }
    ...
    <div className="min-h-screen bg-gray-100 font-sans">
      <header className="bg-white shadow-md">
        <div className="container mx-auto px-4 py-3 flex justify-between items-center">
            <img src="/logo.png" alt="Logo Cabañas Manuara" className="h-12 w-auto" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-6">
                    ...
                    <div>
                        ...
                        <div className="flex items-center space-x-6">
                            <div className="flex items-center"><input id="in_la841" name="flightIn" type="radio" value="LA841" checked={reservation.flightIn === 'LA841'} onChange={handleReservationChange} className="h-4 w-4 text-blue-600" /><label htmlFor="in_la841" className="ml-2 block text-sm">LA841</label></div>
                            <div className="flex items-center"><input id="in_la843" name="flightIn" type="radio" value="LA843" checked={reservation.flightIn === 'LA843'} onChange={handleReservationChange} className="h-4 w-4 text-blue-600" /><label htmlFor="in_la843" className="ml-2 block text-sm">LA843</label></div>
                        </div>
                    </div>
                     <div>
                        <label className="block text-sm font-medium text-gray-600 mb-2">Vuelo de Salida (OUT)</label>
                        <div className="flex items-center space-x-6">
                            <div className="flex items-center"><input id="out_la842" name="flightOut" type="radio" value="LA842" checked={reservation.flightOut === 'LA842'} onChange={handleReservationChange} className="h-4 w-4 text-blue-600" /><label htmlFor="out_la842" className="ml-2 block text-sm">LA842</label></div>
                            <div className="flex items-center"><input id="out_la844" name="flightOut" type="radio" value="LA844" checked={reservation.flightOut === 'LA844'} onChange={handleReservationChange} className="h-4 w-4 text-blue-600" /><label htmlFor="out_la844" className="ml-2 block text-sm">LA844</label></div>
                        </div>
                    </div>
          ...
          <div className="space-y-6">
            ...
            <div className="p-4 bg-white rounded-lg shadow-md border">
                ...
                <button onClick={() => setShowPreview(true)} className="w-full flex justify-center items-center gap-2 px-4 py-3 bg-green-600 text-white font-bold rounded-lg shadow-md hover:bg-green-700 disabled:bg-gray-400" disabled={passengers.length === 0}>
